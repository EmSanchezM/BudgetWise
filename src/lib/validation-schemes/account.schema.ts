...
export const CreateAccountSchemaValidation = {
  ...
  balance: z.coerce.number({ required_error: 'Balance is required' }).min(1, "Balance minimum 1").transform((v) => Math.round(v * 100)),
}
...
export const UpdateAccountSchemaValidation = {
  ...
  balance: z.coerce.number().min(1, "Balance minimum 1").transform((v) => Math.round(v * 100)).nullable(),
}